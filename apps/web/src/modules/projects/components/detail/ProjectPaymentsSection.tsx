...
export function ProjectPaymentsSection({ projectId }: { projectId: string }) {
  ...
  const deleteEvidenceCount = deletingPayment
    ? evidence.filter((e) => e.relatedKind === "payment" && e.relatedId === deletingPayment.id).length
    : 0;
  ...
  return (
    <div id="pembayaran">
      ...
      <ConfirmDialog
        ...
        message={
          <>
            Yakin ingin menghapus pembayaran <strong>{formatCurrency(deletingPayment?.amount ?? 0)}</strong> (
